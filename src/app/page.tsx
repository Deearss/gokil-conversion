...
export default function Home() {
  ...
  return (
    <div
      className={cn(
        "min-h-screen",
        "flex flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950 p-6",
      )}
    >
      {/* HEADER */}
      <div className="text-center mb-8 max-w-xl">
        ...
        <p className="text-zinc-600 dark:text-zinc-400">
          Client-side Distributed Data Processing Simulation. Convert your PNG
          files to highly optimized JPG locally in your browser.
        </p>
      </div>
      ...
      <div className="w-full max-w-3xl grid grid-cols-1 md:grid-cols-2 gap-6 relative">
        ...
        <div className="flex flex-col gap-4">
          <div
            className={cn(
              "flexcc h-75 border-2 border-dashed rounded-2xl transall relative overflow-hidden",
              isDragging
                ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20"
                : "border-zinc-300 dark:border-zinc-800 bg-white dark:bg-zinc-900",
              selectedFile
                ? "border-solid border-zinc-200 dark:border-zinc-800"
                : "hover:border-zinc-400 dark:hover:border-zinc-700 cursor-pointer",
            )}
              ...
              {!selectedFile ? (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transall flexcc">
                    <button
                      onClick={resetAll}
                      className="px-4 py-2 bg-white text-black font-semibold rounded-lg shadow transform hover:scale-105 transall"
                    >
                      Change File
                    </button>
                  </div>
          ...
          <button
            ...
            className={cn(
              ...
              !selectedFile || !!outputSrc
                ? "bg-zinc-400 dark:bg-zinc-800 cursor-not-allowed"
        ...
        <div className="flex flex-col gap-4">
          <div
            className={cn(
              "flexcc h-75 border-2 rounded-2xl transall relative overflow-hidden",
              outputSrc
                ? "border-solid border-green-500/30 bg-green-50/50 dark:bg-green-900/10"
                : "border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50",
            )}
          >
            <AnimatePresence mode="wait">
              {outputSrc ? (
      ...
      <div className="mt-12 w-full max-w-3xl">
        <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-4 border-b border-zinc-200 dark:border-zinc-800 pb-2">
          Recent Conversions
        </h3>
        {history.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {history.slice(0, 3).map((item) => (
              <div
                key={item.id}
                className="flex items-center justify-between p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg"
              >
